...
通过 yt-dlp 的 DouyinIE 实现抖音视频解析，统一转换为内部的 MediaInfo / MediaItem 模型。

说明：
- 依赖用户已安装 yt-dlp（与 BilibiliExtractor 一致）。
...
from ..browser.playwright_sniffer import sniff_douyin_with_playwright
...
class DouyinExtractor(BaseExtractor):
    ...
    async def parse(self, url: str, ctx: DownloadContext) -> MediaInfo:
        ...
        优先尝试使用 Playwright 嗅探无水印直链，失败后回退到 yt-dlp DouyinIE。
        """
        # 1. 先尝试 Playwright 嗅探（headless Chromium）
        try:
            sniffed = await sniff_douyin_with_playwright(url, ctx)
            if sniffed.items:
                logger.info("Using Playwright-sniffed direct source for Douyin.")
                return sniffed
            else:
                logger.info("Playwright 未嗅探到可用直链，将回退到 yt-dlp DouyinIE。")
        except Exception as e:
            logger.warning("Playwright 嗅探 Douyin 失败，将回退到 yt-dlp: %s", e)

        # 2. 回退：使用 yt-dlp 解析（原有逻辑）
        if not check_ytdlp():
            raise YtDlpNotFoundError("yt-dlp is required for Douyin extraction")
